...
describe("Pi todo mapper", () => {
  test("maps todo tool result to TodoListCard items", () => {
    ...
    expect(mapPiTodoToolResult(result)).toEqual({
      type: "todo",
      items: [
        { id: "1", text: "泡一杯咖啡", status: "pending", completed: false },
        { id: "2", text: "检查今日邮件", status: "in_progress", completed: false },
        { id: "3", text: "写一段代码", status: "completed", completed: true },
      ],
    });
  });
  ...
  test("in_progress maps to completed: false (not completed)", () => {
    const result = parseToolResult({
      ...
      details: {
        ...
        params: { action: "list" },
        tasks: [
          { id: 1, subject: "进行中的任务", status: "in_progress", activeForm: "正在检查" },
    ...
    expect(mapPiTodoToolResult(result)).toEqual({
      type: "todo",
      items: [
        { id: "1", text: "进行中的任务", status: "in_progress", completed: false, activeForm: "正在检查" },
        { id: "2", text: "已完成的任务", status: "completed", completed: true },
        { id: "3", text: "待处理的任务", status: "pending", completed: false },
      ],
    });
  });
  ...
  test("filters out deleted tasks", () => {
    ...
    expect(mapPiTodoToolResult(result)).toEqual({
      type: "todo",
      items: [{ id: "2", text: "活着的任务", status: "pending", completed: false }],
    });
  });
  ...
  test("canMapPiTodoToolResult distinguishes renderable snapshots from malformed input", () => {
    ...
    expect(
      canMapPiTodoToolResult({ details: { tasks: [{ id: 1, subject: "x", status: "working" }] } }),
    ).toBe(false);
    expect(
