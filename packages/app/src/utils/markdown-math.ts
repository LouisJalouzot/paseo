...
function mathBlock(
  ...
  for (; closingLine < endLine; closingLine++) {
    const line =
      closingLine === startLine
        ? openingLine.slice(delimiter.opening.length)
        : state.src.slice(
    ...
    contentLines.push(line.slice(0, closingStart));
  ...
  if (
    closingLine >= endLine ||
    content.length === 0 ||
    (closingLine === startLine && trailingContent.length > 0)
  ) {
    return false;
  }
  if (silent) {
...
function findInlineMathClosing(
  ...
  const relativeStart = findUnescapedDelimiter(source.slice(contentStart, maximum), closing);
  return relativeStart === -1 ? null : contentStart + relativeStart;
}
